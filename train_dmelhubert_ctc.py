...
import wandb
from dmelhubert.datasets import DMelHuBERTCTCTrainDataset, train_ctc_collate_fn
from dmelhubert.dmelhubert_ctc import DMelHuBERTCTCArgs
from dmelhubert.scheduler import LinearRampCosineDecayScheduler
from dmelhubert.trainer_ctc import DMelHuBERTCTCLightningModule


@dataclass
class TrainArgs:
    # fmt: off
    project_name: str = "dMelHuBERT-CTC"
    run_name: str = "dmelhubert-iter3-CTC-LibriSpeech-char"
    ...
    train_labels_dir: str = "/workspace/data/ctc-labels/LibriSpeech"
    train_labels_pattern: str = "train*/**/*.pt"
    valid_dmels_dir: str = "/workspace/data/dmel/LibriSpeech"
    valid_dmels_pattern: str = "dev*/**/*.pt"
    valid_labels_dir: str = "/workspace/data/ctc-labels/LibriSpeech"
...
def train(model_args: DMelHuBERTCTCArgs, train_args: TrainArgs) -> None:
    ...
    # DATASET SETUP
    train_dataset = DMelHuBERTCTCTrainDataset(
    ...
    valid_dataset = DMelHuBERTCTCTrainDataset(
    ...
    train_loader = DataLoader(
        ...
        collate_fn=train_ctc_collate_fn,
        pin_memory=True,
        drop_last=False,
    )
    ...
    model = DMelHuBERTCTCLightningModule(model_args)

    checkpoint = torch.load(
        train_args.warm_start_checkpoint_path, weights_only=True, map_location="cpu"
    )
